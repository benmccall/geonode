...
class Migration(DataMigration):
    ...
    # Ensure we have the latest version of base.topiccategory, region, restriction_code_type, and spatial_representation_type
    # and that fixtures have been loaded
    depends_on = (
        ("base", "0018_auto__del_field_topiccategory_layers_count__del_field_topiccategory_do"),
    )
    ...
    def forwards(self, orm):
        ...
        for layer in orm.Layer.objects.all():
            ...
            # Create a new resourcebase for the layer
            layer.resourcebase_ptr = orm['base.ResourceBase'].objects.create()

            # Copy data to the linked resourcebase; these fields are deleted in migration 0009
            layer.resourcebase_ptr.abstract = layer.abstract
            layer.resourcebase_ptr.bbox_x0 = layer.bbox_x0
            layer.resourcebase_ptr.bbox_x1 = layer.bbox_x1
            layer.resourcebase_ptr.bbox_y0 = layer.bbox_y0
            layer.resourcebase_ptr.bbox_y1 = layer.bbox_y1
            layer.resourcebase_ptr.constraints_other = layer.constraints_other
            layer.resourcebase_ptr.csw_anytext = layer.csw_anytext
            layer.resourcebase_ptr.csw_insert_date = layer.csw_insert_date
            layer.resourcebase_ptr.csw_mdsource = layer.csw_mdsource
            layer.resourcebase_ptr.csw_schema = layer.csw_schema
            layer.resourcebase_ptr.csw_type = layer.csw_type
            layer.resourcebase_ptr.csw_typename = layer.csw_typename
            layer.resourcebase_ptr.csw_wkt_geometry = layer.csw_wkt_geometry
            layer.resourcebase_ptr.data_quality_statement = layer.data_quality_statement
            layer.resourcebase_ptr.date = layer.date
            layer.resourcebase_ptr.date_type = layer.date_type
            layer.resourcebase_ptr.distribution_description = layer.distribution_description
            layer.resourcebase_ptr.distribution_url = layer.distribution_url
            layer.resourcebase_ptr.edition = layer.edition
            layer.resourcebase_ptr.language = layer.language
            layer.resourcebase_ptr.maintenance_frequency = layer.maintenance_frequency
            layer.resourcebase_ptr.metadata_uploaded = layer.metadata_uploaded
            layer.resourcebase_ptr.metadata_xml = layer.metadata_xml
            layer.resourcebase_ptr.owner = layer.owner
            layer.resourcebase_ptr.purpose = layer.purpose
            layer.resourcebase_ptr.srid = layer.srid
            layer.resourcebase_ptr.supplemental_information = layer.supplemental_information
            layer.resourcebase_ptr.temporal_extent_end = layer.temporal_extent_end
            layer.resourcebase_ptr.temporal_extent_start = layer.temporal_extent_start
            layer.resourcebase_ptr.title = layer.title
            layer.resourcebase_ptr.uuid = layer.uuid

            # Foreign Keys

            # layer.topic_category is the GeoNode 1.2 topic category charfield
            # layer.category was created as a replacement for layer.topic_category in migration 0002, but was never populated (afaik) and is deleted in 0009
            # layer.resourcebase_ptr.category >base.topiccategory are created in base/migrations/0001 and modified in 0014-0018
            tc = orm['base.TopicCategory'].objects.filter(identifier=layer.topic_category)
            if tc:
                layer.resourcebase_ptr.category = tc[0]

            # layer.resourcebase_ptr.constraints_use is modified in base/migrations/0008-0010
            rct = orm['base.RestrictionCodeType'].objects.filter(identifier=layer.constraints_use)
            if rct:
                layer.resourcebase_ptr.restriction_code_type = rct[0]

            # layer.resourcebase_ptr.spatial_representation_type is modified in base/migrations/0005-0007
